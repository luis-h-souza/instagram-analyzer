...
class ReportService:
    ...
    def gerar_pdf(
        self,
        username: str,
        dados_perfil: Dict[str, Any],
        metricas: Dict[str, Any],
        relatorio_ia: Dict[str, str],
    ) -> str:
        """
        ...
        story.append(
            Paragraph(
                "📊 RELATÓRIO DE ANÁLISE INSTAGRAM", self.styles["TituloPrincipal"]
            )
        )
        ...
        dados_basicos = [
            ...
            [
                "Biografia",
                (
                    dados_perfil.get("biografia", "N/A")[:100] + "..."
                    if len(dados_perfil.get("biografia", "")) > 100
                    else dados_perfil.get("biografia", "N/A")
                ),
            ],
            ...
            [
                "Conta Privada",
                "Sim" if dados_perfil.get("conta_privada", False) else "Não",
            ],
        ]
        ...
        metricas_dados = [
            ...
            [
                "Principais Hashtags",
                ", ".join(metricas.get("principais_hashtags", [])[:5]),
            ],
        ]
        ...
        story.append(
            Paragraph(
                relatorio_ia.get("resumo_negocio", "N/A"), self.styles["TextoNormal"]
            )
        )
        ...
        story.append(
            Paragraph(
                relatorio_ia.get("pontos_fortes", "N/A"), self.styles["TextoNormal"]
            )
        )
        story.append(Spacer(1, 12))
        ...
        story.append(
            Paragraph(
                relatorio_ia.get("pontos_fracos", "N/A"), self.styles["TextoNormal"]
            )
        )
        story.append(Spacer(1, 12))
        ...
        story.append(
            Paragraph(
                relatorio_ia.get("oportunidades", "N/A"), self.styles["TextoNormal"]
            )
        )
        story.append(Spacer(1, 12))
        ...
        story.append(
            Paragraph(
                relatorio_ia.get("sugestao_prospeccao", "N/A"),
                self.styles["TextoNormal"],
            )
        )
        story.append(Spacer(1, 20))
        ...
        if posts:
            for i, post in enumerate(posts[:3], 1):
                ...
                post_info = [
                    [
                        "Legenda",
                        (
                            post.get("legenda", "N/A")[:200] + "..."
                            if len(post.get("legenda", "")) > 200
                            else post.get("legenda", "N/A")
                        ),
                    ],
            ...
            story.append(
                Paragraph(
                    "Nenhum post encontrado para análise.", self.styles["TextoNormal"]
                )
            )
        ...
        story.append(
            Paragraph(
                "Relatório gerado automaticamente pelo Instagram Analyzer",
                self.styles["TextoNormal"],
            )
        )
