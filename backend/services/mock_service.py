...
class MockInstagramService:
    ...
    async def get_profile_data(self, username: str) -> Dict[str, Any]:
        ...
        for i in range(5):
            ...
            post = {
                "id": f"mock_post_{i+1}",
                "caption": f"Post de exemplo #{i+1} #instagram #foto #vida #feliz #amor",
                "likes": curtidas,
    ...
    def calcular_metricas(self, dados_perfil: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Calcular métricas
        total_curtidas = sum(post.get("likes", 0) for post in posts)
        total_comentarios = sum(post.get("comments", 0) for post in posts)
        ...
        principais_hashtags = sorted(
            contador_hashtags.items(), key=lambda x: x[1], reverse=True
        )[:5]
