    ...
    def check_rate_limit(self, username: str) -> tuple[bool, Optional[int]]:
        ...
        if self.global_blocked_until:
            if datetime.now() < self.global_blocked_until:
                wait_time = int((self.global_blocked_until - datetime.now()).total_seconds())
...
async def check_status():
    ...
    return {
        ...
        "rate_limited": is_blocked,
        "blocked_until": blocked_until.isoformat() if is_blocked else None,
        "cache_entries": len(rate_limiter.request_cache),
        "message": "Sistema operacional" if not is_blocked else "Sistema em rate limiting"
    }
...
async def analisar_perfil(username: str, force_mock: bool = False):
    ...
    try:
        ...
        if force_mock:
            ...
            dados_perfil = await mock_service.coletar_dados_perfil(username)
        ...
        if not can_proceed and wait_time > 0:
            ...
            if cached_data:
                ...
                return {
                    **cached_data,
                    "cached": True,
                    "cache_age_minutes": int((datetime.now() - rate_limiter.request_cache[username][1]).total_seconds() / 60)
                }
        ...
        try:
            ...
            dados_perfil = await instagram_service.coletar_dados_perfil(username)
            ...
            if not dados_perfil.get("posts") or len(dados_perfil.get("posts", [])) == 0:
                print(f"⚠️ Perfil coletado mas sem posts - Instagram pode estar bloqueando")
                # Adicionar aviso
            ...
            if "não encontrado" in error_msg or "privado" in error_msg or "not found" in error_msg:
            ...
            elif "rate limiting" in error_msg or "aguarde" in error_msg or "wait" in error_msg:
            ...
            # Acesso bloqueado
            elif "bloqueou" in error_msg or "blocked" in error_msg or "401" in error_msg:
                ...
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": "Acesso ao Instagram temporariamente bloqueado",
                        "retry_after": 1800,
                        "suggestion": "Use /analisar-mock/{username} para dados de demonstração"
                    }
                )
    ...
    try:
        ...
        if use_cache:
            ...
            if cached_data:
                ...
                pdf_path = report_service.gerar_pdf(username, dados_perfil, metricas, relatorio_ia)
                return {
                    "pdf_path": pdf_path, 
